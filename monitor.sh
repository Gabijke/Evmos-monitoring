...
if [ -z "$version" ];
then 
    echo "ERROR: can't find evmos binary">&2 ;
    health=1
    ...
    if [ -z "$status" ];
    then
        echo "ERROR: can't connect to evmos RPC">&2 ;
        health=2
        echo $logentry" health=$health $now"
    else
        # Get block height
        ...
        then
            ...
            # Missing blocks number in window (in evmos slashing window size 100 blocks)
